...
from mips import simulate_data, simple_array, rx_temperature_model, sky_temperature_model
...
def model_run_7():
    ...
    madObj = madrigalWeb.madrigalWeb.MadrigalData('http://millstonehill.haystack.mit.edu/')
    exps = madObj.getExperiments(30,
                                 sd.year,sd.month,sd.day,sd.hour,sd.minute,sd.second,
                                 ed.year,ed.month,ed.day,ed.hour,ed.minute,ed.second)
    exps.sort()
    print(exps[0])

    estart = datetime.datetime(exps[0].startyear,exps[0].startmonth,exps[0].startday,
                               0,0,0)
    ds = sd - estart
    suth = ds.days*24. + ds.seconds/3600.
    de = ed - estart
    euth = de.days*24. + de.seconds/3600.
    ...
    data = madObj.isprint(ef.name, parms, fstr,
                          'Phil Erickson',
    ...
    x_tsys = md['systmp'][0] - rx_temperature_model(fswp,'fixed_medium') - sky_temperature_model(fswp)
    ...
    paramvalues = dict(peak_power_W=pwr,
        ...
        maximum_bulk_doppler=1500.,
        monostatic=True,
        tx_target_rx_angle=0.,
        bistatic_volume_factor=1.,
        quick_bandwidth_estimate=False,
        calculate_plasma_parameter_errors=False)
    paramvalues['O+'] = 1.
    rng_m = np.array(md['gdalt'])*1000

    coorddict= {'gdalt':rng_m/1000, 'tx_to_target_range_m':('gdalt',rng_m), 'target_to_rx_range_m':('gdalt',rng_m), 'Ne':('gdalt',np.array(md['ne'])), 'Te':('gdalt',np.array(md['te'])), 'Ti':('gdalt',np.array(md["ti"])), 'maximum_range_m':('gdalt',rng_m), "maximum_bulk_doppler":('gdalt',vdopp_max)}
    ...
    ax[0].text(2, 450, 'Zenith TX Eff: %.2f\nZenith RX Eff: %.2f\nZenith Tsys: %.1f\nZenith Peak Pwr: %.1f MW' %            (eff_tx, eff_rx, md['systmp'][0], md['power'][0]/1e3))
    ax[0].set_xlabel('SNR')
    ax[0].set_ylabel('Altitude, km')
    ...
    ax[0].semilogx(dataset['measurement_time'].values, md['gdalt'], label='1% Error Time')
    ax[0].grid(True)
    ax[0].legend(fontsize=8)
    ...
    ax[0].legend(fontsize=8)
    ax[1].plot(50e3/dataset['echo_bandwidth'].values, md['gdalt'],label='Model BW / 50 kHz')
